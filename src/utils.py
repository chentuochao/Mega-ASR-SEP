...
def load_net(expriment_config, return_params=False):
    ...
    # params.pl_module = "src.hl_modules.se.Module"
    fabric = L.Fabric(accelerator='cpu')
    params.pl_module_args['init_ckpt'] = None
    if 'init_ckpt_only_big' in params.pl_module_args:
        print("exist")
        params.pl_module_args['init_ckpt_only_big'] = None
    params.pl_module_args['use_compile'] = False
    pl_module = import_attr(params.pl_module)(fabric, **params.pl_module_args)
...
def load_pretrained(run_dir, return_params=False, for_onnx=False, map_location='cpu', use_last=False):
    #print(run_dir)
    ...
    pl_module.load_state(ckpt_path, for_onnx)
