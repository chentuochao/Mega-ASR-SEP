...
class Metrics(nn.Module):
    def __init__(self, name, fs = 16000, **kwargs) -> None:
        ...
        if name == 'snr':
            ...
            self.func = lambda est, gt, mix: snr(preds=est, target=(est-gt))
        elif name == 'snr_i':
            ...
            self.func = lambda est, gt, mix: PESQ(preds=resample_fn(est, fs, fs_new), target=resample_fn(gt, fs, fs_new), fs=fs_new, mode = "wb")
        elif name == 'NBPESQ':
            fs_new = 8000
            resample_fn = lambda x, y, z: x
            if fs_new != fs:
                resample_fn = resample
            self.func = lambda est, gt, mix: PESQ(preds=resample_fn(est, fs, fs_new), target=resample_fn(gt, fs, fs_new), fs=fs_new, mode = "nb")
    ...
    def forward(self, est, gt, mix = None):
        """
        input: (*, C, T)
        output: (*)
        """
        types = type(est)
        assert type(gt) == types and (type(mix) == types or mix is None), "All arrays must be the same type"
        if types == np.ndarray:
