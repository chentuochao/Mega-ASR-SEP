...
class SCOREQ(nn.Module):
    ...
    def _init_pytorch(self):
        ...
        model_key = (self.data_domain, 'nr')#(self.data_domain, self.mode)
    ...
    def calculate_mos(self, audio: torch.Tensor):
        """
        [*, T]
        """
        shape_ = audio.shape[:-1]
        T = audio.shape[-1]
        mos = self.model(audio.reshape(-1, T))

        return mos.reshape(*shape_)

if __name__ == "__main__":
    scoreq = SCOREQ()

    x = torch.randn(4, 2, 80000)
    z = scoreq.calculate_mos(x)
