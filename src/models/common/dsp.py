...
def get_perfect_synthesis_window(analysis_win, lookback, chunksize, lookahead):
    ...
    if (A % B) == 0:
        ...
        print("This case is not handled for perfect window synthesis.\
...
class DualWindowTF(nn.Module):
    def __init__(self, stft_chunk_size, stft_back_pad, stft_front_pad, window_type='hann'):
        super(DualWindowTF, self).__init__()
        
        self.stft_chunk_size = stft_chunk_size
        self.stft_front_pad = stft_front_pad
        self.stft_back_pad = stft_back_pad

        self.nfft = stft_back_pad + stft_chunk_size + stft_front_pad
        ...
        self.synthesis_window = get_perfect_synthesis_window(self.analysis_window, stft_back_pad, stft_chunk_size, stft_front_pad)
    ...
    def init_buffers(self, batch_size, num_channels, device):
        return torch.zeros(batch_size * num_channels, self.istft_lookback, self.synthesis_window.shape[0], device=device)
    ...
    def stft(self, audio_td: torch.Tensor, pad=True):
        ...
        if pad:
            pad_size = (self.stft_back_pad, self.stft_front_pad)
            audio_td, pad_amount = mod_pad(audio_td, chunk_size=self.stft_chunk_size, pad=pad_size)
        ...
        X = torch.stft(audio_td, n_fft = self.nfft, hop_length = self.stft_chunk_size,
                        win_length = self.nfft, window=self.analysis_window,
    ...
    def istft(self, audio_tfd: torch.Tensor, pad_amount=0, istft_buf=None):
        ...
        x = nn.functional.fold(x, output_size=(self.stft_chunk_size * x.shape[-1] + (oWS - self.stft_chunk_size), 1),
                               kernel_size=(oWS, 1), stride=(self.stft_chunk_size, 1)) # [BS, 1, t, 1]
        x = x.squeeze(-1)
        
        x = x[..., -T * self.stft_chunk_size - self.stft_front_pad: - self.stft_front_pad] # [BS, 1, t]
...
def test():
    ...
    backpad = 0
    frontpad = 64
    model = DualWindowTF(chunk_size, backpad, frontpad, 'hann')
    model.eval()

    with torch.inference_mode():
        import soundfile as sf
        x, sr = sf.read('tests/gt.wav')
        x = torch.from_numpy(x[:, 0]).reshape(1,1,-1)
        # x = torch.randn(2, 2, chunk_size * 1000) * 100
        ...
        y, istft_buf = model.istft(X.flatten(1,2), pad)
        print(y.shape)
        print(torch.abs(y - x)[..., chunk_size:].max())
        y = y.reshape(-1).cpu().numpy()

        sf.write('tests/gt_proc.wav', y, sr)
