...
class MLPBlock(nn.Module):
    def __init__(
        ...
        if self.compress_frequencies:
            self.freq_compress = nn.Conv1d(in_channels=latent_dim,
                                out_channels=latent_dim,
                                kernel_size=freq_compression,
                                stride=freq_compression)
            self.act = nn.ReLU()
            
            self.freq_decompress = nn.ConvTranspose1d(in_channels=latent_dim,
                                            out_channels=latent_dim,
                                            kernel_size=freq_compression,
                                            stride=freq_compression,
        ...
        self.intra_seq2seq = MLPStack(
            n_freqs//freq_compression, latent_dim, num_layers=mlp_reps, use_norm=use_norm
        )
    ...
    def forward(self, x, init_state = None, debugDir=None):
        ...
        if self.compress_frequencies:
            intra_rnn = self.freq_compress(intra_rnn.transpose(1, 2)) # [BT, C, K] K = Q // stride
        ...
        if not self.edge:
            ...
            inter_rnn = inter_rnn.reshape([B, Q, T, self.H]).transpose(1, 2) # [B, T, Q, C]
        else:
