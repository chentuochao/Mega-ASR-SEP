    ...
    def __call__(self, audio_data, gt_audio, rng: np.random.RandomState):
        ...
        for i in range(C):
            ...
            if i in self.reference_channels:                
                S = torch.stft(augmented_gt_audio[gt_channel], n_fft=self.nfft, return_complex=True, window=self.window)
                S[mask] = 0
                augmented_gt_audio[gt_channel] = torch.istft(S, n_fft=self.nfft, length=T, window=self.window)
